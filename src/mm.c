 ...
 */
 ...
 {
   /* Set the frame page number and present bit */
   SETBIT(*pte, PAGING_PTE_PRESENT_MASK);
   CLRBIT(*pte, PAGING_PTE_SWAPPED_MASK);
   SETVAL(*pte, fpn, PAGING_PTE_FPN_MASK, PAGING_PTE_FPN_LOBIT);
   return 0;
 ...
 /* 
  * init_pte_simple - Simplified version of init_pte
  * @pte   : target page table entry (PTE)
  * @fpn   : frame page number (FPN)
  */
 int init_pte_simple(uint32_t *pte, int fpn)
 {
   if (fpn < 0)
     return -1;
 
   /* Initialize the page table entry with the frame page number */
   pte_set_fpn(pte, fpn);
 ...
 {                                                   // no guarantee all given pages are mapped
   ...
   // The vmaid field doesn't exist in vm_rg_struct, so we remove this line
